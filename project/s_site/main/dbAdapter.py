...
def add_organization(link, short_name, name, working, address, inn,kpp, okpo, ogrn, okopf, okved, date, profit, costs):
    id = ogrn+inn
    not_done_flag = False
    while not_done_flag == False:
        try:
            import db
            db.add_organization(id, link, short_name, name, working, address, inn,kpp, okpo, ogrn, okopf, okved, date, profit, costs)
            not_done_flag = True

        except Exception as e:
            log("LOGS", "DB_LOG", "Организация НЕ добавлена в БД ", e, "\n")
            print('Организация НЕ добавлена в БД', e)
            time.sleep(0.6)
    pass
